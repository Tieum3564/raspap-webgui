<?php ob_start() ?>
  ...
  <input type="submit" class="btn btn-outline-primary" name="SaveMobileClientSettings" value="<?php echo _('Save settings'); ?>" />
    ...
    <input type="submit" class="btn btn-success" name="StartMobileClient" value="<?php echo _('Connect'); ?>" />
    <?php else : ?>
    <input type="submit" class="btn btn-warning" name="StopMobileClient" value="<?php echo _('Disconnect'); ?>" />
    <?php endif; ?>
  <?php endif ?>
          ...
          <div class="tab-content p-0">
              ...
              <div class="mb-3">
                ...
                <select class="form-select" id="cbxMobileDevice" name="device">
                  <?php foreach ($interfaces as $iface) : ?>
                    <option value="<?php echo htmlspecialchars($iface, ENT_QUOTES, 'UTF-8'); ?>" <?php echo $iface === $settings['device'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($iface, ENT_QUOTES, 'UTF-8'); ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
              <div class="mb-3">
                <label for="txtHilinkHost"><?php echo _('Hilink host'); ?></label>
            ...
            <div class="tab-pane" id="mobileStatus">
              ...
              <table class="table table-sm table-borderless w-auto">
                <tbody>
                  <tr><th scope="row"><?php echo _('Manufacturer'); ?></th><td><?php echo htmlspecialchars($mobileInfo['manufacturer'], ENT_QUOTES, 'UTF-8'); ?></td></tr>
                  <tr><th scope="row"><?php echo _('Device'); ?></th><td><?php echo htmlspecialchars($mobileInfo['device'], ENT_QUOTES, 'UTF-8'); ?></td></tr>
                  <tr><th scope="row"><?php echo _('Connection mode'); ?></th><td><?php echo htmlspecialchars($mobileInfo['mode'], ENT_QUOTES, 'UTF-8'); ?></td></tr>
                  <tr><th scope="row"><?php echo _('Signal quality'); ?></th><td><?php echo htmlspecialchars($mobileInfo['signal'], ENT_QUOTES, 'UTF-8'); ?></td></tr>
                  <tr><th scope="row"><?php echo _('Operator'); ?></th><td><?php echo htmlspecialchars($mobileInfo['operator'], ENT_QUOTES, 'UTF-8'); ?></td></tr>
                  <tr><th scope="row"><?php echo _('IP address'); ?></th><td><?php echo htmlspecialchars($mobileInfo['ipaddress'], ENT_QUOTES, 'UTF-8'); ?></td></tr>
                </tbody>
              </table>
            </div>
            ...
            <div class="tab-pane" id="mobileLogs">
              ...
              <?php if (empty($actionLog)) : ?>
                <p class="text-muted mb-0"><?php echo _('No action output yet.'); ?></p>
              <?php else : ?>
                <pre class="mb-0 p-3 bg-light border rounded" style="max-height: 280px; overflow: auto;"><?php echo implode("\n", $actionLog); ?></pre>
              <?php endif; ?>
            </div>
          </div>
